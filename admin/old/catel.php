...
      $('#catel').DataTable( {
    ...
    $connection=Database::getConnection();
     ...
     $result=$connection->query("SELECT `no_registro`, `id`, `cognome`, `nome`, `data_nascita`, 
                                ...
                                (SELECT id_patrizio1 
                                from patrizi 
                                where id_patrizio2 = id 
                                and id_relazione = 1) as padre,
                                (SELECT id_patrizio1 
                                from patrizi 
                                where id_patrizio2 = id 
                                and id_relazione = 2) as madre  
                                FROM patrizio");                        
    ...
    if(mysqli_num_rows($result) == 0){
      ...
      echo('<table width="100%" class="table table-striped table-bordered dt-responsive nowrap" id="catel" cellspacing="0">');
		...
		while($row=mysqli_fetch_array($result)){
            ...
            echo("<td>".$vivente."</td>"); 
            echo("<td><a href=\"updateCatel.php?mod=update&id=$padre\">".$padre."</a></td>"); 
            echo("<td><a href=\"updateCatel.php?mod=update&id=$madre\">".$madre."</a></td>"); 
